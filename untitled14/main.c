...
void eval_bus_drivers() {
    ...
    if(ldMarmuxcheck == 0)
        ...
        GATEMARMUXVALUE = CURRENT_LATCHES.Vector;
...
void latch_datapath_values() {
    ...
    if(track == 0) {
        ...
        {
            if(CURRENT_LATCHES.MAR >= 0X0000 && CURRENT_LATCHES.MAR <= 0x2FFF &&( ((CURRENT_LATCHES.PSR&0x8000) >>15) ==1) )
                NEXT_LATCHES.Vector = 0x0204;
            else if(CURRENT_LATCHES.MAR %2 != 0)
                NEXT_LATCHES.Vector = 0x0206;
            else if((((CURRENT_LATCHES.IR & 0xF000 )>> 12)== 10) || (((CURRENT_LATCHES.IR & 0xF000 )>> 12)== 11) )
            {
